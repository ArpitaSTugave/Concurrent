...
__global__ void convert(uchar* d_r, uchar* d_g, uchar* d_b, uchar* d_gray, int rows, int columns)
{
    ...
    //int threadid = x + y * gridDim.x * blockDim.x + z * gridDim.x * blockDim.x * gridDim.y * blockDim.y;

    //int blockid = blockIdx.x + blockIdx.y * gridDim.x + blockIdx.z * gridDim.x * gridDim.y;
    //int threadid = threadIdx.x + threadIdx.y * blockDim.x + threadIdx.z * blockDim.x * blockDim.y + blockid * blockDim.x * blockDim.y * blockDim.z;
    int threadid = blockIdx.x * blockDim.x + threadIdx.x;

    if (threadid < rows * columns)
    {
        float grayValue = round(0.299f * float(d_r[threadid]) + 0.587f * float(d_g[threadid]) + 0.114f * float(d_b[threadid]));
        //float grayValue = (d_r[threadid] + d_g[threadid] + d_b[threadid]) / 3.0;
        d_gray[threadid] = (uchar)fminf(grayValue, 254.0f);  // Optional clamp to avoid overflow
    }
}
...
__host__ void executeKernel(uchar *d_r, uchar *d_g, uchar *d_b, uchar *d_gray, int rows, int columns, int threadsPerBlock)
{
    ...
    int totalThreads = rows * columns;
    int blocks = (totalThreads + threadsPerBlock - 1) / threadsPerBlock;

    convert << <blocks, threadsPerBlock >> > (d_r, d_g, d_b, d_gray, rows, columns);
...
int main(int argc, char *argv[])
{
    ...
    try 
    {
        ...
        for(int r = 0; r < rows; ++r)
        {
            for(int c = 0; c < columns; ++c)
            {
                grayImageMat.at<uchar>(r,c) = test_gray[r*rows+c];
            }
        }
        imwrite(outputImage, grayImageMat, compression_params);
